...
@section('page_title')
<h1>服务申请列表 <!--<span class="middle-sep mg-x-10"></span><a href="{{url('/admin/user/new')}}" class="btn btn-info">新建</a>--></h1>
<style type="text/css">
    <style type="text/css">
    /*屏蔽背景层*/
    #divMsgBack
    ...
    <div class="col-md-12 form-group">
            ...
            <form action="" method="get" accept-charset="utf-8">
                <div class="col-md-2">
                    <select class="selectboxit auto-submit" data-target=".submit-btn" name="state">
                        ...
                        <option value='{{Applyservice::APPLY_SUCC}}' @if(Input::get('state') !== '' && Input::get('state') == Applyservice::APPLY_SUCC) selected @endif>申请成功</option>
                        <option value='{{Applyservice::APPLY_FAILED}}' @if(Input::get('state') !== '' && Input::get('state') == Applyservice::APPLY_FAILED) selected @endif>申请失败</option>
                    </select>
                </div>
                <div class="col-md-3">
                    <input type="text" name="time_range" class="form-control daterange" value="{{Input::get('time_range')}}" placeholder="申请时间"/>
                </div>
                <div class="col-md-3">
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <input type="checkbox" name="id" class="chkbox" value="{{$applyservice->id}}">
                    </td>
                    <td class="td-id">{{$applyservice->id}}</td>
                    <td>{{$applyservice->userToHtml()}}</td>
                    <td>{{$applyservice->serviceToHtml()}}</td>
                    <td>{{$applyservice->oneToHtml()}}</td>    <!-- 一级 -->
                    <td>{{$applyservice->twoToHtml()}}</td> <!-- 二级 -->
                    <td>{{$applyservice->stateToHtml()}}</td>
                    <td>{{$applyservice->note}}</td>
                    <td>{{isset($applyservice->created_at) ? $applyservice->created_at->format('Y-m-d H:i') : '无'}}</td>
                    <td class="do">
                        @if($applyservice->state == 0)
                        <a href="{{ url('/admin/user/applyresult') }}?id={{$applyservice->id}}&state={{Applyservice::APPLY_SUCC}}" style="color:#348FCC">申请成功</a>
                        <a href="#" style="color:#CF4339" class="Withdraw_failed" id="{{$applyservice->id}}" state="{{Applyservice::APPLY_FAILED}}">申请失败</a>
                        @endif
                    </td>
                </tr>
                @endforeach
                <tr><td colspan="9">
                    <div class="table-actions row">
                        <div class="pull-right">{{$applyservices->appends(Request::except('page'))->links('Backend.includes.pager')}}</div>
                    </div>
                </td></tr>
                @endif
            </tbody>
...
<script type="text/javascript">
    $(function(){
        ...
        $(".btn-success").click(function(){
            ...
            window.location.href = '/admin/user/applyresult?id='+id+'&state='+state+'&note='+note;
        });
