        ...
        if (! empty ( $tablename )) {
            ...
            while ( $record = mysql_fetch_array ( $data ) ) {
                ...
                if (strlen ( $sql ) >= $size * 1024) {
                    ...
                    if ($this->_write_file ( $sql, $file, $folder )) {
                        $this->_showMsg("表-<b>" . $tablename . "</b>-卷-<b>" . $p . "</b>-数据备份完成,备份文件 [ <span class='imp'>" .$folder . $file ."</span> ]");
                    } else {
            ...
            if ($sql != "") {
                $filename .= "_v" . $p . ".sql";
                if ($this->_write_file ( $sql, $filename, $folder )) {
                    $this->_showMsg( "表-<b>" . $tablename . "</b>-卷-<b>" . $p . "</b>-数据备份完成,备份文件 [ <span class='imp'>" .$folder . $filename ."</span> ]");
                } else {
            ...
            while ( $table = mysql_fetch_array ( $tables ) ) {
                ...
                while ( $record = mysql_fetch_array ( $data ) ) {
                    ...
                    if (strlen ( $sql ) >= $size * 1000) {
                        ...
                        // 写入文件
                        if ($this->_write_file ( $sql, $file, $folder )) {
                            $this->_showMsg("-卷-<b>" . $p . "</b>-数据备份完成,备份文件 [ <span class='imp'>".$folder.$file."</span> ]");
                        } else {
            ...
            if ($sql != "") {
                $filename .= "_v" . $p . ".sql";
                if ($this->_write_file ( $sql, $filename, $folder )) {
                    $this->_showMsg("-卷-<b>" . $p . "</b>-数据备份完成,备份文件 [ <span class='imp'>".$folder.$filename."</span> ]");
                } else {
                    $this->_showMsg("卷-<b>" . $p . "</b>-备份失败",true);
                    return false;
                }
